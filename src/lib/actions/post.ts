 ...
 */
export const createPost = withAuth<typeof createPostSchema, { id: string }>(
  createPostSchema,
 ...
 */
export const updatePost = withAuth<typeof updatePostSchema, { id: string }>(
  updatePostSchema,
  async (data: UpdatePostInput, userId: string) => {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "게시글을 수정하는 중 오류가 발생했습니다."
      );
    }
  }
);
 ...
 */
export const deletePost = withAuth<typeof deletePostSchema, { success: boolean }>(
  deletePostSchema,
  async (data: DeletePostInput, userId: string) => {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "게시글을 삭제하는 중 오류가 발생했습니다."
      );
    }
  }
);
 ...
 * 인증 없이 접근 가능
 */
export const getPosts = withoutAuth(
  z.object({
