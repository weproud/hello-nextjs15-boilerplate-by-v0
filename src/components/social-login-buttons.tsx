...
export function SocialLoginButtons() {
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    try {
      ...
      if (result?.error) {
        toast.error("소설 로그인 실패");
      } else if (result?.url) {
        toast.success("소셜 로그인 성공");
        router.push(result.url);
      }
    } catch (error) {
      toast.error(`소설 로그인 실패: ${error}`);
    } finally {
  ...
  return (
    <div className="flex flex-col space-y-3">
      ...
      <Button
        type="button"
        variant="outline"
        className={`${socialButtonStyles.facebook.bg} ${socialButtonStyles.facebook.text} ${socialButtonStyles.facebook.border} w-full`}
      ...
      >
        {isLoading.facebook ? (
          ...
          socialButtonStyles.facebook.icon
        )}
      ...
      <Button
        type="button"
        variant="outline"
        className={`${socialButtonStyles.kakao.bg} ${socialButtonStyles.kakao.text} ${socialButtonStyles.kakao.border} w-full`}
